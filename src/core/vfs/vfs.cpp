...
namespace
{
  class SourceController
  {
    public:
      ...
      std::vector<char> readFile(const string_view mount, const string_view path)
      {
    ...
    private:
      std::unordered_map<string_view, string> m_Mounts;
  };
  ...
  class VFS
  {
    public:
      ...
      std::vector<char> readFile(const string_view path)
      {
        if (path[0] == '@')
        {
    ...
    private:
      ...
      std::unordered_map<string_view, MountType> m_MountTypes;
  };
...
namespace vfs
{
  void init()
  {
    const ed::Scope& vfsEd = settings::get_app_settings().getScope("vfs");
