...
namespace fs
{
  ...
  void FilesWatcher::watchFileWrites(string_view filePath, Callback&& cb)
  {
    ...
    const auto fileWatchExist = std::find_if(watch.fileWatches.begin(), watch.fileWatches.end(),
    ...
    if (fileWatchExist)
    {
      logerr("FilesWatcher: watch for {} already registered", filePath);
      return;
    }
    ...
    if (watch.handle == nullptr)
    {
      ...
      {
        logerr("FilesWatcher: failed to start watching for dir {} : {}", dir, GetLastError());
        return;
      }
    ...
    watch.fileWatches.push_back(FileWatch{string{fileAbsPath.string()}, std::move(cb)});
  }
    ...
    for (int i = 0; i < watchesCount; ++i)
    {
      ...
      if (dwWaitStatus == WAIT_OBJECT_0)
      {
        ...
        if (FindNextChangeNotification(hndl) == FALSE)
          logerr("FilesWatcher: failed to continue listening notifications for {}", m_This->m_WatchedDirs.getString(id));
      }
    }
  }
}