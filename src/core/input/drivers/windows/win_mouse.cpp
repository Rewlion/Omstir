...
namespace input
{
  ...
  {
    wnd::reg_wnd_proc_handler(this);
    m_Listener = nullptr;

    POINT pos {0,0};
    GetCursorPos(&pos);
    HWND hwnd = (HWND)wnd::get_hwnd();
  ...
  WinMouseDriver::~WinMouseDriver()
  {
    wnd::unreg_wnd_proc_handler(this);
  }
  ...
  void WinMouseDriver::handleWndEvent(void* hwnd,
                                      ...
                                      long long lParam)
  {
    if (msg & wnd::WND_MSG_MOUSE_KEY_MASK)
    {
      unsigned int key = 0;
      switch (wParam)
      {
        case wnd::KEY_LBMOUSE: key = BKEY_LBMOUSE; break;
        case wnd::KEY_RBMOUSE: key = BKEY_RBMOUSE; break;
        case wnd::KEY_MBMOUSE: key = BKEY_MBMOUSE; break;
        default: return;
      }
      const bool pressed = !!(msg & wnd::WND_MSG_BMOUSE_DOWN);
    ...
    else if (msg & wnd::WND_MSG_MOUSE_MOVE)
    {
    ...
    else if (msg & wnd::WND_MSG_MOUSEHWHEEL)
    {
